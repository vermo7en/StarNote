...
it('can Create a Star', async() => {
    let tokenId = 1;
    let instance = await StarNote.deployed();
...
it('lets 2 users exchange stars', async() => {
    ...
    await instance.createStar("great star", 8, {from: user2});
    await instance.exchangeStars(7, 8, {from: user1});
    assert.equal(user1, await instance.ownerOf(8));
...
it('lets a user transfer a star', async() => {
    ...
    assert.equal(user2, await instance.ownerOf(9));
});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let instance  = await StarNote.deployed();
    let user1 = accounts[1];
    await instance.createStar("great star", 10, {from: user1});
    let name = await instance.lookUpTokenIdToStarInfo(10);
    assert.equal("great star", name);
});